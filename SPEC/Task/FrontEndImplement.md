...
1. **网格视图**
   ...
   - 为每个缩略图添加适当大小的标签，覆盖在缩略图的上方，从右到左排列，展示当前图片的质量，对应JSON数据文件中的quality字段。
   - 颜色对应关系：未审查-黄色；优质-绿色；闭眼-红色；模糊-紫色；过暗-橙色；过曝-灰色；需复核-亮蓝色；表情不佳-青色

2. **列表视图**
   - 实现了6列表格：缩略图、名称、大小、修改日期、类型、分辨率、质量
   - 包含5个示例数据行
   - 支持选择行高亮和文件选择信号
